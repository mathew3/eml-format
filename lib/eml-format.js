...
var emlformat = {
  ...
  unquotePrintable: function (s, charset = "utf-8") {
    return contentencodings.decode("qp", s).toString(charset);
  },
};
  ...
  function _read(data) {
    try {
      ...
      if (data.headers["Subject"]) {
        ...
        console.log(emlformat.unquoteString(data.headers["Subject"]));
...
emlformat.parse = function (eml, options, callback) {
  ...
  try {
    ...
    lines.forEach((line) => {
      ...
      if (foundBoundaryStart === false && line.indexOf("--" + boundary) === 0) {

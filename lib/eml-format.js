...
var emlformat = {
  ...
  unquotePrintable: function (s, charset = "utf-8") {
    let c = charset;

    if (charset === "usascii" || charset.includes("iso8859")) {
      c = "ascii";
    }

    return contentencodings.decode("qp", s).toString(c);
  },
};
...
emlformat.build = function (data, options, callback) {
  ...
  try {
    ...
    if (!data.raw && (data.text || data.html || data.attachments)) {
      //Finish the boundary
      eml += "--" + boundary + "--" + EOL;
    }
...
emlformat.parse = function (eml, options, callback) {
  ...
  try {
    ...
    const contentType = result.headers["Content-Type"]
      ? result.headers["Content-Type"].replace(/\s/g, "")
      : "";

    const matchBoundary = /boundary="?(.+?)"?(\s*;[\s\S]*)?$/g.exec(
      contentType
    );

    const boundary = matchBoundary ? matchBoundary[1] || null : null;

    let rawContent = "";

    let foundBoundaryStart = false;
    let foundBoundaryEnd = false;
    let foundFirstEmptyLine = false;

    eml.split(/\r?\n/).forEach((line) => {
      if (!foundFirstEmptyLine && line === "") {
        foundFirstEmptyLine = true;
      }

      if (!!boundary) {
        // ignore lines after closing boundary
        if (foundBoundaryEnd) {
          return;
        }
        ...
        if (
          foundBoundaryStart === false &&
          line.indexOf("--" + boundary) === 0
        ) {
          foundBoundaryStart = true;
          rawContent = rawContent.concat(line, EOL);
        }
        // copy all lines (email body)
        else if (foundBoundaryStart) {
          rawContent = rawContent.concat(line, EOL);
      ...
      if (!insideBody) {
        ...
        if (match) {
          ...
          if (parent.headers[lastHeaderName]) {
            ...
            parent.headers[lastHeaderName].push(value);
          } else {
            //Header first appeared here
            isMultiHeader = false;
            parent.headers[lastHeaderName] = value;
          }
          continue;
        }
      }
      //Body
        ...
        if (isMultipart) {
          ...
          if (
            ...
            // rawContent = rawContent.concat(`--${findBoundary}`, EOL);
          ...
          // console.log('//Solid string body')
          // console.log(lines)

          // lines.forEach(l => {
          //   rawContent = rawContent.concat(l, EOL);
          // })
